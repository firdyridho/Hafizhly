...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'save_score') {
    ...
    // Simpan History
    mysqli_query($conn, "INSERT INTO game_history (user_id, game_type, juz_start, juz_end, total_q, score) VALUES ('$user_id', 'lanjut_ayat', '$juz_start', '$juz_end', '$total_q', '$score')");

    // Cek Achievement Perfect Score
    if ($score == $total_q && $total_q >= 5) {
        $cek_ach = mysqli_query($conn, "SELECT id FROM achievements WHERE user_id='$user_id' AND nama_penghargaan='Master Lanjut Ayat'");
        if (mysqli_num_rows($cek_ach) == 0) {
            mysqli_query($conn, "INSERT INTO achievements (user_id, nama_penghargaan) VALUES ('$user_id', 'Master Lanjut Ayat')");
        }
    }
    exit(json_encode(['status' => 'ok']));
}

// 2. MENGAMBIL DATA LEADERBOARD BULAN INI
$q_leaderboard = mysqli_query($conn, "
    SELECT u.nama_lengkap, SUM(g.score) as total_benar 
    FROM game_history g 
    JOIN users u ON g.user_id = u.id 
    WHERE g.game_type = 'lanjut_ayat' AND DATE_FORMAT(g.created_at, '%Y-%m') = '$current_month' 
    GROUP BY g.user_id 
    ORDER BY total_benar DESC LIMIT 10
");
$leaderboard = [];
...
// 3. MENGAMBIL AKUMULASI PRIBADI
$q_my = mysqli_query($conn, "SELECT SUM(score) as my_score, SUM(total_q - score) as my_wrong, COUNT(id) as play_count FROM game_history WHERE user_id='$user_id' AND game_type='lanjut_ayat' AND DATE_FORMAT(created_at, '%Y-%m') = '$current_month'");
$my_stats = mysqli_fetch_assoc($q_my);
$my_score = $my_stats['my_score'] ?? 0;
$my_wrong = $my_stats['my_wrong'] ?? 0;
$play_count = $my_stats['play_count'] ?? 0;
?>
<!DOCTYPE html>
<html lang="id">
...
<head>
    ...
    <title>Lanjut Ayat - Hifzly</title>
...
<body>
    <div class="container" id="mainScreen">
        <div class="header">
            ...
            <h1 style="font-size:1.5rem; font-weight:800; margin:0;">Lanjut Ayat</h1>
        </div>
        ...
        <div class="my-stats-box" id="myStats">
            <h3 style="font-size:1rem; margin-bottom:10px; color:#065f46;">Statistik Kamu (Bulan Ini)</h3>
            <div style="display:flex; gap:15px; font-size:0.9rem; font-weight:600;">
                <div><i class="fas fa-gamepad"></i> Main: <?= $play_count ?>x</div>
                <div style="color:#059669;"><i class="fas fa-check"></i> Benar: <?= $my_score ?></div>
                <div style="color:#ef4444;"><i class="fas fa-times"></i> Salah: <?= $my_wrong ?></div>
            </div>
            <button onclick="this.parentElement.style.display='none'" style="margin-top:10px; padding:5px 10px; border-radius:8px; border:1px solid #059669; background:white; color:#059669; cursor:pointer; font-weight:600;">Tutup</button>
        </div>

        <div class="card" style="padding:15px;">
            <?php if (empty($leaderboard)): ?>
                <div style="text-align:center; padding:20px; color:var(--text-muted);">Belum ada yang bermain bulan ini. Jadilah yang pertama!</div>
            <?php else: ?>
    ...
    <script>
        ...
        async function initGame() {
            ...
            try {
                ...
                for (let i = 0; i < totalQ; i++) {
                    ...
                    // Jangan ambil ayat terakhir dari array agar ada ayat sambungannya
                    let rIdx = Math.floor(Math.random() * (ayahs.length - 1));
                    const qAyah = ayahs[rIdx];
                    const cAyah = ayahs[rIdx + 1]; // Jawaban benar

                    // Ambil 3 jawaban salah acak dari Juz yang sama
                    let wrongAyahs = [];
                    while (wrongAyahs.length < 3) {
                        let wIdx = Math.floor(Math.random() * ayahs.length);
                        if (wIdx !== (rIdx + 1) && !wrongAyahs.includes(ayahs[wIdx].text)) {
                            wrongAyahs.push(ayahs[wIdx].text);
                        }
                    }

                    // Acak Opsi (1 Benar, 3 Salah)
                    let options = [{
                        t: cAyah.text,
                        v: 'benar'
                    }, {
        ...
        function nextSlide() {
            // Hentikan audio jika masih menyala
            ...
            if (currentSlide < questions.length) {
                ...
                if (nextAudio) nextAudio.play().catch(e => console.log('Autoplay blocked'));
            } else {
                finishGame();
            }
        }
        ...
        function finishGame() {
            ...
            questions.forEach((q, i) => {
                ...
                reviewHtml += `
                    <div class="ri-item" style="border-color:${isBenar ? '#34d399' : '#f87171'};">
                        <div style="font-size:0.9rem; font-family:'Plus Jakarta Sans'; text-align:left; color:${isBenar ? '#059669' : '#ef4444'}; margin-bottom:10px; font-weight:700;">
                            ${isBenar ? '<i class="fas fa-check-circle"></i> Tepat!' : '<i class="fas fa-times-circle"></i> Keliru'}
                        </div>
                        <div style="color:var(--text-muted); font-size:1.1rem; margin-bottom:5px;">${q.q_text} ۝</div>
                        <div style="color:var(--dark);">${q.correct_text} ۝</div>
                    </div>
                `;
            });
            ...
            reviewArea.innerHTML = `
                <div class="score-circle" style="background: ${nilai>=80 ? '#059669' : (nilai>=50 ? '#fbbf24' : '#ef4444')}">${nilai}</div>
                <h2 style="font-size:1.8rem; text-align:center; margin-bottom:10px;">${nilai>=80 ? 'Masya Allah! 🎉' : 'Tetap Semangat! 💪'}</h2>
                <p style="text-align:center; color:var(--text-muted); margin-bottom:20px;">Kamu berhasil menyambung ${skorBenar} ayat dengan benar dari total ${questions.length} ayat.</p>
                ...
                <h3 style="margin-top:30px; font-size:1.1rem; text-align:center; font-family:'Plus Jakarta Sans'; border-bottom:1px solid #e2e8f0; padding-bottom:10px;">Review Ayat</h3>
                ${reviewHtml}
            `;
            reviewArea.style.display = 'block';

            // AJAX Simpan Skor ke DB
        ...
        function shareWA(benar, total) {
            const text = `Alhamdulillah! Saya baru saja menyelesaikan Tantangan Lanjut Ayat di aplikasi Hifzly dan berhasil menjawab benar ${benar} dari ${total} ayat. Yuk uji hafalanmu juga! 🌟`;
